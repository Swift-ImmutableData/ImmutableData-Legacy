...
extension Listener {
  fileprivate func onChange(_ onChange: @escaping @Sendable () -> Void) -> AnyCancellable {
    self.publisher.sink() { _ in
      onChange()
    }
  }
}
...
extension ListenerTests {
  @Test func outputDidChange() async throws {
    ...
    let listener = await Listener<StateTestDouble, ActionTestDouble, Never, OutputTestDouble>(
    ...
    do {
      ...
      await confirmation(expectedCount: 1) { onChange in
        let cancellable = await listener.onChange(onChange.confirm)
